...
const collectionOfDays=document.getElementsByClassName("desk-bookings__desk ng-star-inserted");
...
function getUser(){
    if(collectionOfDays.length===1){
        ...
        chrome.storage.local.set({'day':collectionOfDays[0].parentElement.firstChild.textContent});
        chrome.storage.local.set({'deskno':collectionOfDays[0].firstChild.firstChild.firstChild.nextElementSibling.firstChild.nextElementSibling.textContent})
    }
    else{
         for(let i=0;i<collectionOfDays.length;i++){
            if(i!=collectionOfDays.length-1){
                ...
                deskNo=deskNo+collectionOfDays[i].firstChild.firstChild.firstChild.nextElementSibling.firstChild.nextElementSibling.textContent+','
